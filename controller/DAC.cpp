...
static byte i2cwrite(uint8_t i2cAddress, uint8_t command, uint16_t data)
{
...
int Dac::set_all_current(float f)
{
  ...
  byte status = i2cwrite(AD5645R_BROADCAST_ADDR,commandByte,DacSetInt);
...
int Dac::set_current(int device, int channel, float f)
{
  ...
  byte status = i2cwrite(m_devices[device],commandByte,DacSetInt);
